...
const (
	clientName    = "SSE-Roots-Example-Client"
	clientVersion = "1.0.0"
	serverURL     = "http://localhost:3002/mcp/sse"
)

func main() {
	log.Printf("Starting MCP SSE Roots Example Client...")
	...
	// Create SSE client connecting to our server
	log.Printf("Connecting to SSE server: %s", serverURL)
...
func startInteractiveMode(ctx context.Context, client *mcp.Client, rootsProvider *mcp.DefaultRootsProvider) {
	...
	fmt.Println()
	fmt.Println("=== MCP SSE Roots Interactive Demo ===")
...
func testServerTools(ctx context.Context, client *mcp.Client) {
	...
	fmt.Println("   root directory information from the client via SSE transport.")
}
