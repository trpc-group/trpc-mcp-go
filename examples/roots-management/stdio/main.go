...
// STDIO MCP Roots Client Example.
package main
...
const (
	clientName    = "STDIO-Roots-Example-Client"
	clientVersion = "1.0.0"
)

func main() {
	log.Printf("Starting MCP STDIO Roots Example Client...")
	...
	// Create STDIO client
	log.Printf("Creating STDIO client...")
	mcpClient, err := mcp.NewStdioClient(
		config,
		clientInfo,
	...
	// === Send Roots Change Notification ===
	log.Printf("Sending roots list changed notification...")
	err = mcpClient.SendRootsListChangedNotification(ctx)
	if err != nil {
		log.Printf("Failed to send roots list changed notification: %v", err)
	} else {
		log.Printf("Successfully sent roots list changed notification")
	}
...
func setupRootsProvider(client *mcp.StdioClient) *mcp.DefaultRootsProvider {
...
func startInteractiveMode(ctx context.Context, client *mcp.StdioClient, rootsProvider *mcp.DefaultRootsProvider) {
	scanner := bufio.NewScanner(os.Stdin)

	fmt.Println()
	fmt.Println("=== MCP STDIO Roots Interactive Demo ===")
...
func sendRootsChangedNotification(ctx context.Context, client *mcp.StdioClient) {
...
func testServerTools(ctx context.Context, client *mcp.StdioClient) {
	...
	tools := []struct {
		...
		{"count_roots", "Count client's root directories", map[string]interface{}{}},
		{"list_client_roots", "List all client's root directories", map[string]interface{}{}},
		{"root_stats", "Get statistics about client's roots", map[string]interface{}{}},
	}
	...
	fmt.Println("   root directory information from the client via STDIO transport.")
}
