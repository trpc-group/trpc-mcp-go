...
func handleCompletions(ctx context.Context, client *mcp.Client) error {
	...
	if err != nil {
		...
		log.Printf("Prompt completion for '%s' with prefix '%s': found %d suggestions",
	...
	if err != nil {
		...
		log.Printf("Resource completion from '%s' for '%s': found %d suggestions",
	...
	resourceTemplateCompletionReq.Params.Ref.URI = "file://test"
