...
func (c *Client) SendRootsListChangedNotification(ctx context.Context) error {
	...
	case *sseClientTransport:
		return transport.sendNotification(ctx, notification)
	case *streamableHTTPClientTransport:
		return transport.sendNotification(ctx, notification)
	case *stdioClientTransport:
		return transport.sendNotification(ctx, notification)
	default:
		return fmt.Errorf("SendRootsListChangedNotification not supported for transport type %T", transport)
	}
}
